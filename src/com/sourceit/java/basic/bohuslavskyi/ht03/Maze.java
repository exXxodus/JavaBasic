...
public class Maze extends java.applet.Applet implements Runnable {
	...
	int[][] maze; // Description of state of maze. The value of maze[i][j]
					...
					// cell that is part of a cooridor is represented by
					// pathCode
					// if it is part of the current path through the maze, by
					// visitedCode if it has already been explored without
					// finding
					// a solution, and by emptyCode if it has not yet been
					// explored.
	...
	Color[] color = new Color[5]; // colors associated with the preceding 5
									// constants;
	int rows = 21; // number of rows of cells in maze, including a wall around
					// edges
	int columns = 21; // number of columns of cells in maze, including a wall
						// around edges
	...
	boolean mazeExists = false; // set to true when maze[][] is valid; used in
								// redrawMaze(); set to true in createMaze(),
								// and
	...
	void makeMaze() {
		...
		for (i = 1; i < rows - 1; i += 2)
			...
			for (j = 1; j < columns - 1; j += 2) {
				emptyCt++;
				maze[i][j] = -emptyCt; // each room is represented by a
		...
		for (i = wallCt - 1; i > 0; i--) {
			r = (int) (Math.random() * i); // choose a wall randomly and maybe
											// tear it down
			if (checkStatus() == TERMINATE)
				return;
